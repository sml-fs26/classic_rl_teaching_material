...
*Block 1 of 3 · 45 min · the formalism for sequential decisions under uncertainty.*
...
::: {.column width="45%"}
![](figures/agent_env_loop.png){width="100%" fig-align="center"}
:::
::::
...
Everything the agent needs to know to act. **Warehouse robot:**

![](figures/state_factorization.png){width="78%" fig-align="center"}
...
What the agent can choose at each step. **Warehouse robot:**

![](figures/actions_arrows.png){width="78%" fig-align="center"}

::: {.fragment}
Action availability can depend on the state — `pick-up` only makes sense on the package cell. Either give $\mathcal{A}(s)$ per state, or let illegal actions take a large negative reward.
:::
...
A battery-powered shuttle moves along a one-way 3-stop bus route. At each stop it can `wait` (passengers may board) or `depart`. Each passenger gives reward $+5$; each minute waiting costs $-1$; arriving at the depot ends the episode.
...
![](figures/bus_route.png){width="78%" fig-align="center"}
...
*Done: Block 1 (MDP). Up next: Block 2 — value functions and the Bellman equation.*

# Block 2 — Returns, value functions, Bellman {.section}

*Block 2 of 3 · 50 min · what we want from a policy, and how it constrains itself.*
...
$$
{\color{#2563eb}v_\pi(s)} \;=\; \sum_{\color{#f97316}a} {\color{#f97316}\pi(a \mid s)} \sum_{s'} P(s' \mid s, a) \Bigl[\, {\color{#dc2626}R} + \gamma\, {\color{#2563eb}v_\pi(s')} \,\Bigr]
$$
...
::: {.column width="35%"}
![](figures/backup_v.png){width="85%" fig-align="center"}
:::
::::
...
$$
{\color{#2563eb}q_\pi(s, a)} \;=\; \sum_{s'} P(s' \mid s, a) \Bigl[\, {\color{#dc2626}R} + \gamma\, {\textstyle\sum_{\color{#f97316}a'}} {\color{#f97316}\pi(a' \mid s')}\, {\color{#2563eb}q_\pi(s', a')} \Bigr]
$$
...
::: {.column width="35%"}
![](figures/backup_q.png){width="85%" fig-align="center"}
:::
::::
...
$$
\begin{aligned}
q_\pi(s, \mathrm{N}) \;=\; -1 \,+\, \gamma\, \Bigl[ \;
& 0.8\, q_\pi(s_N, \mathrm{N}) \\
+\; & 0.1\, q_\pi(s_W, \mathrm{N}) \\
+\; & 0.1\, q_\pi(s_E, \mathrm{N}) \;\Bigr]
\end{aligned}
$$
...
*Done: Blocks 1 & 2 (MDP, Bellman). Up next: Block 3 — SARSA itself.*

# Block 3 — Exploration and SARSA {.section}

*Block 3 of 3 · 50 min · turn the Bellman equation into a learning algorithm.*
...
★ ${\color{#2563eb}Q(S,A)} \leftarrow {\color{#2563eb}Q(S,A)} + \alpha\bigl[\,{\color{#dc2626}R} + \gamma\,{\color{#2563eb}Q(S', A')} - {\color{#2563eb}Q(S,A)}\bigr]$
...
*10 min · what we built, and where to go next.*
