...
::: {.column width="45%"}
<!-- FIGURE: 4×4 gridworld with robot, package cell, depot cell, and slippery cells highlighted. Generated in figures/warehouse.png -->
:::
::::
...
Future rewards are *discounted* by $\gamma$ per step.
...
::: {.fragment .callout-warning title="Pre-empt"}
A *large* $\gamma$ does **not** mean a short-term focus. Quite the opposite — $\gamma$ close to $1$ makes the agent **patient**.
:::
...
$$
\pi(a \mid s) \;=\; \Pr\!\left[\,A_t = a \,\big|\, S_t = s\,\right]
$$
...
A 3-state chain. Actions: `left`, `right`. Transitions deterministic. Step cost $-1$; entering the terminal $T$ gives reward $+10$. Discount $\gamma = 0.9$. Policy $\pi$ = "always `right`."

<!-- FIGURE: linear chain  s_1 ⇄ s_2 → T  with arrows for `left` and `right`; rewards −1 per step, +10 on entering T -->
...
q_\pi(s_1, \mathrm{left})  &= -1 + \gamma\, q_\pi(s_1, \mathrm{right})
\end{aligned}
$$
:::
...
::: {.fragment}
There is always a deterministic optimal policy $\pi_*$. Its action-value satisfies the **Bellman optimality equation:**
$$
...
::: {.fragment}
Only difference vs. Bellman expectation: $\sum_{a'} \pi(a'|s')$ → $\max_{a'}$. We won't dwell on $q_*$; the rest of the lecture **estimates $q_\pi$ for a gradually-improving policy**.
:::
...
::: {.fragment .callout-important}
Of course you bump it now. That is **bootstrapping**.
This is the heart of TD — and you don't need an MDP to feel it.
:::
...
\text{uniform random action} & \text{with probability } \varepsilon
\end{cases}
$$
...
::: {.fragment .callout-tip}
> *"Are you sure you've chosen the best door?"*  — David Silver, UCL Lecture 5
:::
...
**Constant $\varepsilon$** (e.g. $0.1$). Simple. Works in many problems. But the policy stays *forever* stochastic, so it can never reach the optimum.

::: {.fragment}
**Decaying $\varepsilon$** (e.g. $\varepsilon_t = 1/t$). Explore early; exploit late.
:::

::: {.fragment .callout-note title="GLIE"}
*Greedy in the Limit with Infinite Exploration.* A schedule that:
- visits every $(s, a)$ infinitely often, **and**
- becomes greedy in the limit.
...
$$
\underbrace{S}_{\text{state}} \;\;\longrightarrow\;\; \underbrace{A}_{\text{action}} \;\;\longrightarrow\;\; \underbrace{R}_{\text{reward}} \;\;\longrightarrow\;\; \underbrace{S'}_{\text{next state}} \;\;\longrightarrow\;\; \underbrace{A'}_{\text{next action}}
$$
...
<!-- FIGURE: three snapshots of the warehouse gridworld, each cell quartered with Q-values written in. Captioned: episode 0 (Q ≡ 0), episode 50 (some structure), episode 500 (clear value flow toward depot). -->
...
A $7 \times 10$ grid. **Wind** blows the agent upward by 1 or 2 cells in the middle columns, regardless of the chosen action. Start `S`, goal `G`. Step cost $-1$.

<!-- FIGURE: Windy Gridworld: 7×10 grid with start S, goal G, and wind strength 0/1/2 indicated below each column. Trajectory of trained SARSA agent overlaid. Side panel: learning curve (episodes on x-axis, time-steps on y-axis), characteristic concave-down shape. -->

::: {.fragment}
**SARSA** with $\alpha = 0.5$, $\varepsilon = 0.1$, $\gamma = 1$:

- Finds a working path in $\sim 100$ episodes.
- Stable solution: $\sim 17$ steps per episode.
- Optimal (with this wind, no $\varepsilon$): $15$ steps. The extra $2$ are the cost of exploration.
:::
...
**SARSA:**
$$
Q(S, A) \;\leftarrow\; Q(S, A) + \alpha \,\Bigl[\, R + \gamma\, \mathbf{Q(S', A')} - Q(S, A)\, \Bigr]
$$
...
**Q-learning:**
$$
Q(S, A) \;\leftarrow\; Q(S, A) + \alpha \,\Bigl[\, R + \gamma\, \mathbf{\max_{a'} Q(S', a')} - Q(S, A)\, \Bigr]
$$
...
<!-- FIGURE: Cliff Walking grid. Bottom row marked CLIFF. Two trajectories overlaid: SARSA's path goes one row up (safe), Q-learning's path hugs the cliff edge (optimal but risky). Side panel: per-episode reward curve, Q-learning oscillates lower, SARSA higher and more stable. -->

::: {.fragment}
- **SARSA** learns the **safe** path — one row above the cliff. It *accounts for* the $\varepsilon$-greedy slip that occasionally pushes it off.
